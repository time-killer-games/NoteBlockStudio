function save_song() {
	...
	if ((!backup) && (fn = "" || filename_ext(cursong.filename) != ".nbs")) {
	    ...
	    fn = string(get_save_filename_ext_dynamic("Note Block Songs (*.nbs)|*.nbs", fsave + condstr(filename_ext(cursong.filename) != ".nbs", ".nbs"), songfolder, condstr(language !=1, "Save song", "保存歌曲")))
