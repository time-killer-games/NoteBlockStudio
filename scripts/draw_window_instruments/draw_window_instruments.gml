function draw_window_instruments() {
	...
	if (draw_button2(x1 + 456, y1 + 318, 80, "OK") && wmenu = 0 && (windowopen = 1 || theme != 3)) {
		...
		if (save_version < 5 && cursong.user_instruments > 18) {
			show_message_dynamic("This song contains more than 18 instruments and cannot be saved in version " + string(save_version) + ". The save version will be changed to " + string(nbs_version) + ".")
	...
	if (draw_button2(x1 + 456, y1 + 318, 80, "确定") && wmenu = 0 && (windowopen = 1 || theme != 3)) {
		windowclose = 1
		if (save_version < 5 && cursong.user_instruments > 18) {
			show_message_dynamic("此歌曲含有多于18个音色，无法保存为版本 " + string(save_version) + "。将保存为版本 " + string(nbs_version) + "。")
			save_version = nbs_version
		}
		save_settings()
	}
