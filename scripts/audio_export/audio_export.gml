function mp3_export() {
	...
	fn = string(get_save_filename_ext_dynamic(output_format + " files (*" + output_ext + ")|*" + output_ext, filename_new_ext(songs[song].filename, "") + output_ext, filename_path(songs[song].filename), condstr(language != 1, "Export audio track", "导出音频文件")))
	if (fn = "") return 0
	...
	try {
		...
		if (language != 1) show_message_dynamic("An error occurred while exporting the song:\n\n" + e)
		else show_message_dynamic("导出歌曲时发生错误：\n\n" + e)
		return -1;
	}
