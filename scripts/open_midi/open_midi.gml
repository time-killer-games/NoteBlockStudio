function open_midi() {
	...
	if (fn = "") {
		if (language != 1) fn = string(get_open_filename_ext_dynamic("MIDI Sequences (*.mid)|*.midi;*.mid", "", "", "Import from MIDI"))
		else fn = string(get_open_filename_ext_dynamic("MIDI Sequences (*.mid)|*.midi;*.mid", "", "", "从 MIDI 导入"))
	}
