function datapack_getinstextures() {
	...
	var fn, src
	if (language != 1) fn = string(get_save_filename_ext_dynamic("Resource pack (*.zip)|*.zip", "Note Block Textures", "", "Replaces instrument blocks with note block textures."))
	else fn = string(get_save_filename_ext_dynamic("Resource pack (*.zip)|*.zip", "音符盒纹理", "", "用自定义纹理替换音色方块"))
	if (fn = "") return 0
