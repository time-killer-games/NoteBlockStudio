function datapack_export() {
	...
	if (language != 1) {
	if (o.dat_usezip) fn = string(get_save_filename_ext_dynamic("ZIP archive (*.zip)|*.zip", dat_name + ".zip", "", "Data Pack Export"))
	else fn = string(get_save_filename_ext_dynamic("Data Pack Folder", dat_name, "", "Data Pack Export"))
	} else {
	if (o.dat_usezip) fn = string(get_save_filename_ext_dynamic("ZIP archive (*.zip)|*.zip", dat_name + ".zip", "", "导出数据包"))
	if (o.dat_usezip) fn = fn + condstr(filename_ext(fn) != ".zip", ".zip")
	else fn = string(get_save_filename_ext_dynamic("数据包目录", dat_name, "", "导出数据包"))
	}
	if (fn = "") return 0
